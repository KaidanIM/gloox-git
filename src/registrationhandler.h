...
class RegistrationHandler
{
  public:
    ...
    enum resultEnum
    {
      ...
      REGISTRATION_NOT_AUTHORIZED,          /*!< Account removal: Unregistered entity waits too long before
                                             * authentication or performs tasks other than authentication after
                                             * registration.<br>
                                             * Password change: The server or service does not consider the
                                             * channel safe enough to enable a password change. */
      REGISTRATION_BAD_REQUEST,             /*!< Account removal: The &lt;remove/&gt; element was not the only
                                             * child element of the &lt;query/&gt; element. Should not happen
                                             * when only gloox functions are being used.<br>
                                             * Password change: The password change request does not contain
                                             * complete information (both &lt;username/&gt; and
      ...
      REGISTRATION_UNEXPECTED_REQUEST,      /*!< Account removal: The host is an instant messaging server and
                                             * the IQ get does not contain a 'from' address because the entity
                                             * is not registered with the server.<br>
                                             * Password change: The host is an instant messaging server and
                                             * the IQ set does not contain a 'from' address because the entity
    ...
    /**
     * This function is called if @ref Registration::createAccount() was called on an authenticated stream
     * and the server lets us know about this.
     */
